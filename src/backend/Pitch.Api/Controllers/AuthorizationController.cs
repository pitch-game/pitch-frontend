...
namespace AuthorizationServer.Controllers
{
    public class AuthorizationController : Controller
    {
        ...
        public async Task<IActionResult> Authorize(OpenIdConnectRequest request)
        {
            ...
            var user = await _userService.GetOrCreate(User.FindFirstValue(ClaimTypes.Name), User.FindFirstValue(ClaimTypes.Email));
            return CreateIdentityAndSignIn(user);
        }

        /// <summary>
        /// Return a token to a temporary account that has the lifetime of a token and cannot be logged into again
        /// For demonstration purposes
        /// TODO Limit somehow to prevent abuse
        /// Set TTE on entity in cosmosdb
        /// </summary>
        /// <returns></returns>
        [HttpGet("~/connect/throwaway")]
        public async Task<IActionResult> Throwaway()
        {
            var user = await _userService.CreateThrowaway();
            return CreateIdentityAndSignIn(user);
        }

        private IActionResult CreateIdentityAndSignIn(User user)
        {
